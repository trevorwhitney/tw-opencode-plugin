...
digraph process {
    ...
    "Read plan, extract tasks, create TodoWrite, record base SHA (git rev-parse HEAD)" [shape=box];
    ...
    "Read plan, extract tasks, create TodoWrite, record base SHA (git rev-parse HEAD)" -> "Dispatch implementer subagent (./implementer-prompt.md)";
...
**Cost:**
- More subagent invocations (implementer + 2 reviewers per task + multi-model final review)
